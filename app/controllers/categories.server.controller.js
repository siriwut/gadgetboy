 ...
 */
 exports.create = function(req, res) {
 	...
 	if(req.body.parent) category.parent = req.body.parent._id;
 	...
 	category.save(function(err) {
 		if (err){
 			...
 			if(category.parent){
 				Category.buildSubs(category._id,category.parent,function(err,parent){
 					if(err){
 						return res.status(400).send({
 							message: errorHandler.getErrorMessage(err)
 						});
 ...
 */
 exports.categoryByID = function(req, res, next, id) { 
 	Category.findById(id).populate('user', 'displayName').populate('parent').populate('subs').exec(function(err, category) {
